...
export function MobilePreview() {
  ...
  if (formTabs.length === 0) {
    return (
      <div className="w-64 h-[500px] border rounded-lg flex items-center justify-center bg-background">
        <p className="text-sm text-muted-foreground text-center px-4">Add tabs and form elements to see a preview</p>
      </div>
    )
  }
  ...
  const currentTabId =
    activePreviewTab && formTabs.some((tab) => tab.id === activePreviewTab) ? activePreviewTab : formTabs[0]?.id || null
            ...
            <div className="space-y-1">
              {(element.properties.options || ["Option 1", "Option 2"]).map((option, index) => (
                <div key={index} className="flex items-center space-x-2">
                  <input type="checkbox" id={`preview-${element.id}-${index}`} className="scale-75" />
                  <Label htmlFor={`preview-${element.id}-${index}`} className="text-xs">
                    {option}
                  </Label>
                </div>
              ))}
      ...
      case "select":
        return (
          <div className="space-y-1" key={element.id}>
            ...
            <select id={`preview-${element.id}`} className="w-full p-1 border rounded-md bg-background text-xs h-8">
      ...
      case "file":
        return (
          <div className="space-y-1" key={element.id}>
            ...
            <Input id={`preview-${element.id}`} type="file" className="h-8 text-xs" />
          </div>
        )
      case "image":
        return (
  ...
  return (
    <div className="w-64 h-[500px] border rounded-[20px] overflow-hidden flex flex-col bg-background shadow-md">
      ...
      <div className="flex-1 overflow-hidden p-2">
        {formTabs.length > 1 ? (
          <Tabs value={currentTabId || undefined} onValueChange={setActivePreviewTab} className="w-full">
            <TabsList className="w-full mb-2 h-8">
              {formTabs.map((tab) => (
                <TabsTrigger key={tab.id} value={tab.id} className="text-xs h-6">
                  {tab.title}
                </TabsTrigger>
              ))}
            </TabsList>
            {formTabs.map((tab) => (
              <TabsContent key={tab.id} value={tab.id} className="mt-0 data-[state=active]:overflow-auto h-[430px] pb-14">
                <div className="space-y-3 p-2">{tab.elements.map((element) => renderFormElement(element))}</div>
              </TabsContent>
            ))}
          </Tabs>
        ) : (
          <div className="h-[430px] overflow-auto">
            <div className="space-y-3 p-2 pb-14">{formTabs[0]?.elements.map((element) => renderFormElement(element))}</div>
          </div>
        )}
      </div>
